 ...
 *
 * Please see added Print function.
 *
...
LinkedListUnsortedType::LinkedListUnsortedType(
		const LinkedListUnsortedType &list) {
	length = list.length;
...
ItemType LinkedListUnsortedType::GetItem(ItemType &item, bool &found)
...
// Post: No element in the list has a key that matches item's.
	...
	if (item.ComparedTo(listData->info) == CompareRelation::SAME) {
		tempLocation = location;
		listData = listData->next;		// Delete first node.
	} else {
		while (item.ComparedTo((location->next)->info) != CompareRelation::SAME)
			location = location->next;
...
ItemType LinkedListUnsortedType::GetNextItem()
